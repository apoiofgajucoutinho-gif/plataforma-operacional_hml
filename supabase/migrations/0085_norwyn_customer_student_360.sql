-- P1 Customer & Student 360: consolidated operational view over existing P0.3 foundation.
...
with eligible_sales as (
  ...
  select
    ...
    min(e.access_started_at) as first_access_at,
...
select
  ...
  coalesce(la.club_first_access_at, ea.first_access_at) as first_access_at,
  coalesce(la.club_last_event_at, ea.last_access_at) as last_access_at,
  case
    when coalesce(ea.enrollment_count,0) = 0 then 'NOT_APPLICABLE'
    when coalesce(la.learning_event_count,0) = 0 and ea.first_access_at is null and ea.last_access_at is null then 'UNKNOWN_ACCESS'
    when coalesce(la.club_last_event_at, ea.last_access_at) >= now() - interval '45 days' then 'ACTIVE'
    else 'INACTIVE'
  ...
  case
    when coalesce(ea.enrollment_count,0) = 0 then 'NOT_APPLICABLE'
    when coalesce(la.club_first_access_at, ea.first_access_at) is not null then 'IN_PROGRESS'
    else 'UNKNOWN'
  ...
  case
    ...
    when coalesce(ea.enrollment_count,0) > 0 and coalesce(la.learning_event_count,0) = 0 and ea.first_access_at is null then 'ACESSO_DESCONHECIDO'
