...
type ViewMode = "specialist" | "admin" | "links";
...
export function PresenceCenterPage({ context }: Props) {
  ...
  const selectedIncidents = selectedAsset ? context.incidents.filter((incident) => incident.asset_id === selectedAsset.id) : [];
  ...
  return (
    <div className="space-y-6">
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        ...
        <MetricCard label="Incidentes hoje" value={String(incidents.filter((incident) => incident.detected_at.startsWith(new Date().toISOString().slice(0, 10))).length)} icon={Clock3} tone="neutral" />
      </div>
  ...
  return (
    <Surface>
      ...
      <div className="mt-5 rounded-[22px] border border-[color:var(--ds-border)] bg-[color:var(--ds-bg-soft)] p-4">
        ...
        {tab === "content" ? <List title="Conteudo e score" items={scoreExplanation} empty="Sem explicacao de score registrada ainda." /> : null}
        ...
        {tab === "incidents" ? <List title="Incidentes do ativo" items={incidents.map((incident) => `${incident.status} · ${incident.severity} · ${incident.title}`)} empty="Nenhum incidente registrado para este ativo." /> : null}
