...
import { discoverInternalLinks } from "@/modules/presence/services/presence-monitor";
...
export async function POST(request: Request) {
  ...
  if (!access.isAdmin) return NextResponse.json({ error: "Apenas Admin pode executar discovery" }, { status: 403 });
  ...
  const result = await discoverInternalLinks(access.dataClient, asset);
  return NextResponse.json({ message: `${result.discoveredCount} sublink(s) descoberto(s) para revisao`, ...result });
}